...
def grade_assignment(p, incoming_payload):
    grade_assignment_payload = GradeAssignmentSchema().load(data=incoming_payload)
    graded_assignment = Assignment.submit(
        _id=grade_assignment_payload.id, grade=grade_assignment_payload.grade, principal=p, flag="grade"
    )
    db.session.commit()
